...
async def generate_christmas_poem_handle(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    chat_mode = 'christmas poem assistant'
    db.set_user_attribute(user_id, "current_chat_mode", chat_mode)
    await message_handle(update, context, message='Generate text about christmas', use_new_dialog_timeout=False)
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        BotCommand("/christmas_poem", "Generate christmas poem"),
...
def run_bot() -> None:
    ...
    application.add_handler(CommandHandler("christmas_poem", generate_christmas_poem_handle, filters=user_filter))
